...
If any changed files are under `mkwineprefix/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `mkwineprefix/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `mkwineprefix/commands/main.py`,
...
For Python files, strip the `mkwineprefix/` prefix and replace `/` with `.` (like module imports).

- Python file `mkwineprefix/media.py` → `media:`.
- Multiple files under `mkwineprefix/commands/` → `commands:`.
- Single command file `mkwineprefix/commands/admin.py` → `commands.admin:`.
