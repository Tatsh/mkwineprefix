...
Fixes mypy type errors and eliminates `Any` usage in the mkwineprefix project.
...
1. Fix all mypy errors reported by `uv run mypy mkwineprefix`
...
Place new TypedDict classes in `mkwineprefix/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`mkwineprefix/typing.py` when shared.
...
1. Run `uv run mypy mkwineprefix` and capture all errors.
...
4. Run `uv run mypy mkwineprefix` again to verify fixes.
